const diagram = (svg, candidates, states) => {
  ...
  const update = (selectedState) => {
    ...
    svg.append('line')
      ...
      .attr('y1', 0)
      .attr('y2', (radius * 2) + (stateRadius * 2))
      .attr('stroke', '#000')
    ...
    const simulation = d3.forceSimulation(candidatesNodes)
      .force('x', d3.forceX().strength(candidateRadius / 10000))
      .force('y', d3.forceY().strength(candidateRadius / 10000))
      .force('center', d3.forceCenter(radius + stateRadius * 1.5, radius + stateRadius * 1.5))
      .force('collide', d3.forceCollide(candidateRadius * 1.5).strength(Math.sqrt(candidates.length) / 40))
      .on('tick', () => {
